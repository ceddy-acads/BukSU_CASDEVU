...
<div class="page-head">
    <div>
        <h1>My activities</h1>
        <p><?= $activeCount ?> active registration<?= $activeCount === 1 ? '' : 's' ?></p>
    </div>
    <a class="btn btn-gold" href="<?= url('activities/index.php') ?>">Browse activities</a>
</div>
            ...
            <div style="display:flex;flex-wrap:wrap;gap:1rem;justify-content:space-between;align-items:flex-start;">
                <div style="flex:1 1 320px;">
                    ...
                    <?php if ($registration['status'] === 'rejected' && $registration['review_remarks']): ?>
                        <div class="alert alert-error" style="margin:.75rem 0 0;">
                ...
                <div style="flex:0 1 260px;">
                    <?php if ($progress['total'] > 0): ?>
                        <div class="hint" style="margin-bottom:.35rem;">
                            Requirements verified: <strong><?= $progress['satisfied'] ?></strong>
                            of <?= $progress['total'] ?>
                        </div>
                        <?php if ($needsFiles): ?>
                            <a class="btn btn-gold btn-sm btn-block"
                               href="<?= url('requirements/submit.php?activity_id=' . (int) $registration['activity_id']) ?>">
                                <?= $progress['missing'] ?> still needed — submit now
                            </a>
                        <?php else: ?>
                        ...
                        <?php endif; ?>
                    <?php else: ?>
                        <p class="hint" style="margin:0;">No documents are required.</p>
                    <?php endif; ?>
                </div>
            </div>
        </section>
